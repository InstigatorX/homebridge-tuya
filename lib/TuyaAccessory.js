...
class TuyaAccessory extends EventEmitter {
    ...
    _connect() {
        ...
        (this._socket.reconnect = () => {
            consoledebuglog(`[Tuya DEBUG] reconnect called for ${this.context.name}`);
    ...
    _msgHandler_3_1(task, callback) {
        ...
        if (this.context.intro === false && cmd !== 9)
            console.debug('[Tuya] Message from', this.context.name + ':', data);
        ...
        switch (cmd) {
            ...
            case 8:
                ...
                if (data && data.dps) {
                    //console.debug('[Tuya] Update from', this.context.name, 'with command', cmd + ':', data.dps);
    ...
    _msgHandler_3_3(task, callback) {
        ...
        switch (cmd) {
            ...
            case 10:
                if (data) {
                    if (data.dps) {
                        console.debug(`[Tuya] Heard back from ${this.context.name} with command ${cmd}`);
                        this._change(data.dps);
                    } else {
    ...
    update(o) {
        ...
        let result = false;
        if (hasDataPoint) {
            console.debug("[Tuya] Sending", this.context.name, JSON.stringify(dps));
            result = this._send({
                data: {
            ...
            if (result !== true) console.debug("[Tuya] Result", result);
            if (this.context.sendEmptyUpdate) {
                console.debug("[Tuya] Sending", this.context.name, 'empty signature');
                this._send({cmd: 7});
            }
        } else {
            console.debug(`[Tuya] Sending first query to ${this.context.name} (${this.context.version})`);
            result = this._send({
                data: {
